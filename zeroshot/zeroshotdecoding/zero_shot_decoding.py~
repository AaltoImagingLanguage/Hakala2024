...
def zero_shot_decoding(X, y, verbose=True, break_after=None, normalize=True,
    ...
    model : scikit-learn regression model | None
        The model to use. If None, the RidgeCV model, given in ridge.py
    ...
    # model to use
    if model is None:
        model = RidgeCV(alphas=np.logspace(-5, 5, 100), alpha_per_target=True,
...
def predictions_breakdown(accuracies, category_assignments, category_labels,
    ...
    if verbose:
        print(f'{accuracy}\t{n_comparisons}\tOverall accuracy')
    ...
    for i, category_code in enumerate(category_codes):
        ...
        if verbose:
            print(f'{accuracy}\t{n_within_comparisons[i]}\t'
                  f'Within {category_labels[category_code]}')
    ...
    for i in range(n_categories):
        for j in range(i + 1, n_categories):
            ...
            if verbose:
                print(f'{accuracy}\t{n_between_comparisons[ind]}\t'
                      f'{category_labels[category_codes[i]]} vs '
                      f'{category_labels[category_codes[j]]}')
            ind = ind + 1
    ...
    if verbose:
        print(f'{overall_within_cat_accuracy}\t{np.sum(n_within_comparisons)}'
              '\tOverall within category')
        print(f'{overall_between_cat_accuracy}\t{np.sum(n_between_comparisons)}'
              '\tOverall between category')
